...
bkgForge is a provider-neutral Rust workspace for constructing reproducible workflows and operating them through replaceable resources. The current delivery includes the M2 trace foundation: versioned, sanitized capture records and replay contracts, without website fetching, browser capture, or generated artifacts.

## Architecture

```text
core ← workflow, resources, lanes, security, discovery, trace
                         ↑
                      runtime ← CLI
```
...
- Persisted traces use a documented versioned line format and redact sensitive HTTP headers before storage or replay.
