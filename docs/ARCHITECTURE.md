...
This document records the repository state after M2, **Trace Foundation**. It is not a claim that HTTP or browser capture, endpoint discovery, schemas, or generators exist.
...
| `bkgforge-trace` | Versioned capture trace, sensitive-header redaction, in-memory persistence port, and replay input contract. | Round trip/replay, version rejection, sanitization, and store/replay contract. |
...
bkgforge-core
  ↑        ↑         ↑        ↑         ↑         ↑
workflow resources lanes security discovery trace
  ↑        ↑         ↑        ↑         ↑         ↑
  └──────────── bkgforge-runtime ──────┘
                        ↑
                 bkgforge-cli
...
The trace crate owns a documented, versioned line format with metadata, navigation records, paired request/response exchanges, and events. It accepts only `Real` or `Simulation` provenance for a newly produced trace; replay consumes an immutable `ReplayInput`. Before serialization, in-memory storage, or replay loading, `Authorization`, `Cookie`, `Set-Cookie`, and `X-API-Key` headers are redacted case-insensitively.

This preserves the established architecture: the domain does not instantiate a browser, perform HTTP I/O, store cookies, or retain secret values. A future discovery executor must use the request's declared execution mode, workflow identity, trace contract, and generic resource requirements rather than bypassing runtime/resource policy.
...
- M2 defines capture records but does not capture network traffic; M3 owns real HTTP capture.
